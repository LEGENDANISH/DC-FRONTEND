...
interface IncomingCallProps {
  call: {
    ...
    type: 'voice' | 'video';
    timestamp: number;
    caller?: User; // Make caller optional since it might not be in the socket data
  };
  onAccept: () => void;
  onDecline: () => void;
}
...
const IncomingCallNotification: React.FC<IncomingCallProps> = ({ 
  ...
  // Create a caller object from available data
  const caller = call.caller || {
    id: call.callerId,
    username: call.callerId, // Fallback to using ID as username
...
export default function CallInterface({ currentUser, isOpen, onClose }: CallInterfaceProps) {
  ...
  const getOtherParticipant = (): User | null => {
    ...
    // Determine the other participant based on call data
    const otherUserId = activeCall.callerId === currentUser.id 
      ? activeCall.targetUserId 
      : activeCall.callerId;
    
    // Create a basic user object - in a real app you'd fetch this from your user store/API
    return {
      id: otherUserId,
